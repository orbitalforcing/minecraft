...
for i in range(int(width/2) + 1):
    ...
    # gable ends
    if (int(width/2) -i > 0):
        (x+1+i, y+height+i, z+3, x+width-i-1, y+height+i, z+3, block.BRICK_BLOCK.id, 0)
        (x+1+i, y+height+i, z+3+depth, x+width-i-1, y+height+i, z+3+depth, block.BRICK_BLOCK.id, 1)
